...
const ProductDetail = () => {
  const [pet, setPet] = useState(null); // Changed initial state to null
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        ...
        const petData = response.data; // Removed unnecessary await
        setPet(petData);
      } catch (error) {
        ...
        <div className="lg:w-4/5 mx-auto flex flex-wrap">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex py-10">
              <Link to={localStorage.getItem("token") ? `/petadopt/${pet._id}`:("/signin")}>
                <button className="flex text-white text-xl bg-indigo-800 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded" onClick={handleEnquiry}>Enquire About {pet.name}</button>
              </Link>
